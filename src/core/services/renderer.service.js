import * as processors from 'app_modules/processors';


const processorByName = [processors].reduce((acc, procesorLike) => {

    Object.keys(procesorLike).forEach(key => {
        let processor = procesorLike[key];
        acc[processor.name] = processor;
    });

    return acc;
}, {});

export class RendererService {

    _iterateDomWithProcessors(target, cb) {
        let domList = Array.prototype.slice.call(target.children);
        domList.forEach(node => {
            let processors = node.getAttribute('data-processors');
            if (processors) {
                cb(node, processors);
            } else {
                this._iterateDomWithProcessors(node, cb);
            }
        });
    }

    process(target) {
        // INIT
        this._iterateDomWithProcessors(target, (node, processors) => {
            node._processed = node._processed || {};
            node._children = node._children || [];
            node._parent = node._parent || target;
            node._processorInstances = node._processorInstances || {};
            node._processorParams = node._processorParams || {};
            let nodeProcessors = processors.split(/,\s*/).map(k => processorByName[k]);

            nodeProcessors.forEach(processor => this.applyProcessor(node, processor));

            if (target._children) {
                target._children.push(node);
            }
            this.process(node);
        });
        // PROCESS
        this._iterateDomWithProcessors(target, (node) => {
            for (let processorName in node._processed) {
                let processor = processorByName[processorName];
                let instance = this._getProcessorInstance(processor, node);
                processor.process(instance, node._processorParams[processorName]);
            }
        });
    }

    applyProcessor(node, processor) {
        let params = node.getAttribute('data-processor-' + processor.name);
        if (params) {
            params = JSON.parse(params);
        }

        node._processorParams[processor.name] = params;
        let instance = this._getProcessorInstance(processor, node);

        if (!node._processed[processor.name]) {
            processor.preInit(instance, params);
            processor.init(instance, params);
            node._processed[processor.name] = true;
        }
        // processor.process(instance, params); // This one should work on dom refresh - but I don't have any right now
    }


    _getProcessorInstance(processor, node) {
        if (node._processorInstances[processor.name]) {
            return node._processorInstances[processor.name];
        }
        let instance = node._processorInstances[processor.name] = {node, processor};

        processor._class.instanceMethods.forEach(function (methodName) {
            instance[methodName] = processor[methodName].bind(instance);
        });
        return instance;
    };

    clear(target) {
        this._iterateDomWithProcessors(target, node => {
            if (node._processed) {
                for (let processorName in node._processed) {
                    let processor = processorByName[processorName];
                    let instance = this._getProcessorInstance(processor, node);

                    processor.destroy(processor, node, node._processorParams[processor.name]);
                    this.destroy(node);
                    if (node._parent._children) {
                        let ind = node._parent._children.indexOf(node);
                        if ( ind >= 0 ) {
                            node._parent._children.splice(ind, 1);
                        }
                    }
                    delete node._parent;
                    processor.postDestroy(instance);
                }
            }
            while (node.firstChild) {
                node.removeChild(node.firstChild);
            }
        });
    };

    destroy(target) {
        this.clear(target);

        if (target.parentNode) {
            target.parentNode.removeChild(target);
        }
    };
}

export const rendererService = new RendererService();
